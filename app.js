/* Conversa Live v2.5.2 — estabilidade, mídia e refinamento geral */
...
(function(){
 const $=id=>document.getElementById(id),
       ...
       applyTheme=()=>{const t=localStorage.getItem("conversaLiveTheme")||"dark",a=localStorage.getItem("conversaLiveAccent")||"purple";document.documentElement.dataset.theme=t;document.documentElement.dataset.accent=a;const b=$("themeToggle");if(b){b.querySelector("span")?.replaceChildren(document.createTextNode("Tema"));b.title="Tema: "+a;const icons={purple:"☀️",blue:"🔵",cyan:"🩵",green:"🟢",orange:"🟠",pink:"🩷"};b.firstChild?.replaceWith(document.createTextNode(t==="dark"?icons[a]||"🎨":"🌙"));}},
       ...
       cycleAccent=()=>{const a=localStorage.getItem("conversaLiveAccent")||"purple",i=THEME_ACCENTS.indexOf(a),n=THEME_ACCENTS[(i+1)%THEME_ACCENTS.length];localStorage.setItem("conversaLiveAccent",n);applyTheme();appToast?.("Tema: "+n,"info");};
 applyTheme();
 document.addEventListener("DOMContentLoaded",()=>{applyTheme();$("themeToggle")?.addEventListener("click",e=>{if(e.shiftKey)toggleTheme();else cycleAccent();});});
...
/* FreeChat 2.5.1 — Calls 3.0 / media helpers */
...
function applyCallVolumeState(){
 ...
 document.querySelectorAll("#videos .tile video").forEach(v=>{if(v.closest(".local-tile"))return;v.muted=callVolumeMuted;v.volume=callVolumeLevel;});
