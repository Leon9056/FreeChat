/* Conversa Live v2.6.0 — estabilidade, mídia e refinamento geral */
...
let peers=new Map(),pendingRemoteIce=new Map(),people=new Map(),inCall=false;
...
function openFeed(){const panel=$("socialPanel");if(!panel)return;panel.classList.remove("hidden");panel.classList.add("feed-mode");switchSocialTab("feed");loadFeed();document.body.classList.add("feed-open");}
function closeSocialPanel(){$("socialPanel")?.classList.add("hidden");$("socialPanel")?.classList.remove("feed-mode");document.body.classList.remove("feed-open");}
...
function applyCallVolumeState(){
 ...
 document.querySelectorAll("#videos .tile video").forEach(v=>{if(v.closest(".local-tile")||v.dataset.local==="1")return;v.muted=callVolumeMuted;v.volume=callVolumeLevel;});
