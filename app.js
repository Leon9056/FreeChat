/* Conversa Live v1.9.5 — UI/UX, amigos e mensagens diretas */
...
function showApp(d){
 ...
 $("welcomeName").textContent=d.user.name;$("myCode").textContent=d.user.code;$("sideCode").textContent=d.user.code;window.renderFriends?.();
}
...
window.renderFriends=async()=>{
  let d;
  try{d=await api("/api/friends")}catch(e){d={friends:[],requests:[]}};
  window.friendDirectory=d;
  const lists=[$("friendsList"),$("friendsAppList")].filter(Boolean);
  lists.forEach(list=>{
    list.innerHTML="";
    const term=String(window.friendSearchTerm||"").trim().toLowerCase();
    const friends=(d.friends||[]).filter(u=>!term||String(u.name||"").toLowerCase().includes(term)||String(u.code||"").toLowerCase().includes(term));
    const requests=d.requests||[];
    if(list.id==="friendsList" && requests.length){
      const title=document.createElement("div");title.className="friends-section-title";title.innerHTML='<span>Solicitações</span><small>'+requests.length+'</small>';list.appendChild(title);
      requests.forEach(u=>{
        ...
        x.querySelector(".friend-avatar").textContent=String(u.name||"?").trim().charAt(0).toUpperCase();
        x.querySelector("b").textContent=u.name||"Usuário";
        x.querySelector("small").textContent=u.code+" • quer ser seu amigo";
        x.querySelector(".accept-friend-btn").onclick=async()=>{await api("/api/friends/accept",{method:"POST",body:JSON.stringify({code:u.code})});await refreshUnreadCounts();
$("friendsSearch")?.addEventListener("input",e=>filterFriends(e.target.value));
$("friendsSearchApp")?.addEventListener("input",e=>filterFriends(e.target.value));renderFriends()};
        x.querySelector(".reject-friend-btn").onclick=async()=>{try{await api("/api/friends/reject",{method:"POST",body:JSON.stringify({code:u.code})})}catch(e){}renderFriends()};
        list.appendChild(x);
      });
    }
    if(friends.length){
      const title=document.createElement("div");title.className="friends-section-title";title.innerHTML='<span>Seus amigos</span><small>'+friends.length+'</small>';list.appendChild(title);
    }
    friends.forEach(u=>{
      const online=[...people.values()].some(p=>p.code===u.code);
      const unread=Number(unreadCounts[u.code]||0);
      const x=document.createElement("div");x.className="friend-item";
      x.innerHTML='<div class="friend-avatar"></div><span class="friend-dot"></span><div class="friend-info"><b></b><small></small></div><span class="friend-unread" hidden></span><button class="message-friend-btn" title="Mensagem direta">💬</button><button class="friend-call-btn" title="Iniciar call">📞</button><button class="remove-friend-btn" title="Remover amigo">×</button>';
      x.querySelector(".friend-avatar").textContent=String(u.name||"?").trim().charAt(0).toUpperCase();
      x.querySelector(".friend-dot").classList.toggle("online",online);
      x.querySelector("b").textContent=u.name||"Usuário";
      x.querySelector("small").textContent=online?"● Online":"○ Offline";
      const badge=x.querySelector(".friend-unread");
      if(unread){badge.textContent=unread>99?"99+":String(unread);badge.hidden=false}
      x.querySelector(".message-friend-btn").onclick=()=>openMessages(u);
      x.querySelector(".friend-call-btn").onclick=()=>{const c=makeCallCode();openApp(c,true)};
      x.querySelector(".remove-friend-btn").onclick=async()=>{if(!confirm("Remover "+(u.name||"este amigo")+" da sua lista?"))return;await api("/api/friends/remove",{method:"POST",body:JSON.stringify({code:u.code})});delete unreadCounts[u.code];renderFriends()};
      list.appendChild(x);
    });
    if(!friends.length && !(list.id==="friendsList" && requests.length)){
      const empty=document.createElement("div");empty.className="friends-empty";
      empty.innerHTML='<div class="friends-empty-icon">👥</div><b>'+ (term?"Nenhum amigo encontrado":"Sua lista está vazia") +'</b><small>'+ (term?"Tente outro nome ou código.":"Adicione amigos pelo código para começar.") +'</small>';
