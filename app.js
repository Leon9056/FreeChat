/* Conversa Live v2.3.0 — áudio/música melhores, animações fluidas, otimizações e correções de bugs */
...
function startSocket(){
  ...
  socket.on("dm-new",({code:fromCode,message})=>{
...
/* FreeChat 2.0.8 — mensagens diretas */
let activeFriendCode=null, messageTimer=null, lastLoadedMessages=[];
...
function renderMessagesList(scroll=true){
 ...
 const msgs=term?lastLoadedMessages.filter(m=>String(m.body||"").toLowerCase().includes(term)):lastLoadedMessages;
 if(msgs.length){
   list.innerHTML=msgs.map(m=>{
     const mine=String(m.sender_id)===String(window.CONVERSA_USER?.id);
     return `<div class="message-bubble ${mine?"mine":"theirs"}"><div>${messageEscape(m.body)}</div><small>${formatMessageTime(m.created_at)}</small></div>`;
   }).join("");
 }else{
   list.innerHTML=term?'<div class="muted">Nenhuma mensagem encontrada para "'+messageEscape(term)+'".</div>':'<div class="muted">Nenhuma mensagem ainda. Diga olá! 👋</div>';
 }
...
async function loadMessages(scroll=true){
 ...
 try{
  const d=await api("/api/messages/"+encodeURIComponent(activeFriendCode));
...
window.openMessages=function(friend){
 ...
 unreadCounts[friend.code]=0;
 activeFriendCode=friend.code;lastLoadedMessages=[];
 $("messagesFriendName").textContent=(friend.name||friend.code)+" • "+friend.code;
 $("messagesPanel").classList.remove("hidden");$("messageStatus").textContent="";if($("messagesSearch"))$("messagesSearch").value="";$("messageInput").focus();loadMessages(true);
};
...
messageTimer=setInterval(()=>{if(!window.CONVERSA_TOKEN)return;refreshUnreadCounts();if(activeFriendCode&&!$("messagesPanel")?.classList.contains("hidden"))loadMessages(false)},5000);

document.addEventListener("DOMContentLoaded",()=>{$("friendsSearch")?.addEventListener("input",e=>{window.friendSearchTerm=e.target.value;renderFriends()});$("messagesSearch")?.addEventListener("input",()=>renderMessagesList(false))});
