/* Conversa Live v2.5.0 — áudio/música melhores, animações fluidas, otimizações e correções de bugs */
...
/* FreeChat 2.5.0 — navegação por gesto no celular */
let mobileView=0;
...
/* FreeChat 2.5.0 — Calls 3.0 / media helpers */
...
let selectedPostMedia=null,callStatsTimer=null,callChatHistory=[],callFocusedTile=null;
function clearPostMedia(){selectedPostMedia=null;const i=$("postMedia");if(i)i.value="";const p=$("postMediaPreview");if(p){p.innerHTML="";p.classList.add("hidden")}}
function renderPostMediaPreview(file){
 ...
 else{const v=document.createElement("video");v.controls=true;v.playsInline=true;v.src=URL.createObjectURL(file);wrap.appendChild(v)}
...
openCall=async function(){
 ...
 if(inCall){$("callRoomLabel")?.replaceChildren(document.createTextNode("# "+room));startCallStats();updateCallParticipantCount()}
};
...
/* FreeChat 2.5.0 — Social, notificações e PWA */
...
/* FreeChat 2.5.0 — chat privado com fotos, vídeos e notificações */
...
/* FreeChat 2.5.0 — estabilidade global, rede e mobile polish */
(function(){
