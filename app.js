...
function openSettings(){const p=$("settingsPanel");if(!p)return;p.classList.remove("hidden");renderThemeChoices();}
function closeSettings(){$("settingsPanel")?.classList.add("hidden");}
function initSettings(){
 $("settingsBtn")?.addEventListener("click",openSettings);$("settingsClose")?.addEventListener("click",closeSettings);
...
async function createPeer(id,initiator){
  ...
  // Create media transceivers first so codec preferences actually apply.
  localStream.getTracks().forEach(t=>pc.addTransceiver(t,{direction:"sendrecv",streams:[localStream]}));
  try{
    const caps=RTCRtpSender.getCapabilities?.("audio");
    if(caps?.codecs){
      const opus=caps.codecs.filter(c=>/opus/i.test(c.mimeType));
      const rest=caps.codecs.filter(c=>!opus.includes(c));
      pc.getTransceivers().filter(t=>t.sender?.track?.kind==="audio").forEach(t=>{try{t.setCodecPreferences([...opus,...rest])}catch(e){}});
    }
  }catch(e){}
  ...
  pc.ontrack=e=>{
    ...
    if(e.track.kind==="audio"){
      ...
      if(v){
        ...
        v.autoplay=true;
        v.playsInline=true;
        v.volume=callVolumeMuted?0:callVolumeLevel;
        const p=v.play();
        if(p?.catch)p.catch(()=>{
...
function enterOrExitTileFocus(tile){
  ...
  if(id==="local"){video.muted=true;video.volume=0;video.play().catch(()=>{});}
  else{video.muted=callVolumeMuted;video.volume=callVolumeMuted?0:callVolumeLevel;video.play().catch(()=>{});}
