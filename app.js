/* Conversa Live v1.8 — 2FA por e-mail + UI/UX aprimorada */
...
(function(){
 ...
 let mode="login",pendingChallenge="",pendingEmail="",pendingPassword="",resendTimer=null;
 ...
 function modeSet(m){
   ...
   $("authSubtitle").textContent=m==="register"?"Crie sua conta e confirme seu e-mail.":"Entre na sua conta para continuar";
   setStatus("");$("twoFactorFields").classList.add("hidden");pendingChallenge="";clearInterval(resendTimer);resendTimer=null;$("resend2faBtn").disabled=false;$("resend2faBtn").textContent="📨 Reenviar código";
 }
 function maskEmail(v){const [local,domain]=String(v||"").split("@");if(!domain)return v;const shown=local.length<=2?local[0]||"*":local.slice(0,2);return shown+"***@"+domain}
 function show2FA(d,em,pw){
   pendingChallenge=d.challenge;pendingEmail=em;pendingPassword=pw;
   ["loginTab","registerTab","registerFields","emailLabel","passwordLabel","loginBtn","passwordStrength","confirmPasswordWrap"].forEach(id=>$(id)?.classList.add("hidden"));
   email.classList.add("hidden");password.closest(".password-wrap")?.classList.add("hidden");
   $("authSubtitle").textContent="Quase lá — confirme seu e-mail";$("twoFactorFields").classList.remove("hidden");
   $("twoFactorHelp").textContent="Digite o código de 6 dígitos enviado para "+maskEmail(em)+". Ele expira em 10 minutos.";
   $("twoFactorCode").value="";$("twoFactorCode").focus();setStatus(d.message||"Código enviado para seu e-mail.","ok");startResendCountdown(60);
 }
 function restoreAuth(){
   email.classList.remove("hidden");password.closest(".password-wrap")?.classList.remove("hidden");
   ["emailLabel","passwordLabel","loginTab","registerTab","loginBtn"].forEach(id=>$(id)?.classList.remove("hidden"));
   modeSet("login");email.focus();
 }
 function showApp(d){localStorage.setItem("conversaLiveToken",d.token);localStorage.setItem("conversaLiveUser",JSON.stringify(d.user));window.CONVERSA_TOKEN=d.token;window.CONVERSA_USER=d.user;login.classList.add("hidden");menu.classList.remove("hidden");$("welcomeName").textContent=d.user.name;$("myCode").textContent=d.user.code;$("sideCode").textContent=d.user.code;window.renderFriends?.()}
 ...
 function updatePasswordStrength(){const p=password.value,score=passwordScore(p),bar=$("strengthBar"),text=$("strengthText");if(!bar||!text)return;const labels=["Muito fraca","Fraca","Razoável","Boa","Forte"];bar.style.width=(p?score*25:0)+"%";text.textContent=p?labels[score]:"Digite uma senha";bar.dataset.score=score;text.dataset.score=score}
 function togglePasswordField(inputId,buttonId){const input=$(inputId),btn=$(buttonId);if(!input||!btn)return;const visible=input.type==="password";input.type=visible?"text":"password";btn.setAttribute("aria-pressed",String(visible));btn.setAttribute("aria-label",visible?"Ocultar senha":"Mostrar senha");btn.classList.toggle("visible",visible);btn.querySelector(".eye-icon")?.replaceChildren(document.createTextNode(visible?"◉":"◉"));}
 async function auth(){
   ...
   try{const r=await fetch(serverUrl()+"/api/"+(mode==="register"?"register":"login"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(mode==="register"?{name:nm,email:em,password:pw}:{email:em,password:pw})});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||"Não foi possível concluir a operação.");show2FA(d,em,pw)}
   catch(e){setStatus(e.message||"Erro ao conectar ao servidor.");}
   finally{if(!$("twoFactorFields").classList.contains("hidden")){btn.disabled=true}else setBusy(btn,false,mode==="register"?"✨ Criar conta":"🚀 Entrar")}
 }
 async function verify2FA(){
   const c=$("twoFactorCode").value.replace(/\D/g,"");if(!/^[0-9]{6}$/.test(c)){setStatus("Digite o código de 6 dígitos.");return}
   const btn=$("verify2faBtn");setBusy(btn,true,"Verificando...");setStatus("Validando seu código...","loading");
   try{const r=await fetch(serverUrl()+"/api/2fa/verify",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({challenge:pendingChallenge,code:c})});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||"Código inválido.");clearInterval(resendTimer);showApp(d)}catch(e){setStatus(e.message||"Código inválido.");$("twoFactorCode").select()}finally{setBusy(btn,false,"Verificar código")}
 }
 function startResendCountdown(seconds){clearInterval(resendTimer);let left=seconds;const btn=$("resend2faBtn");btn.disabled=true;const tick=()=>{if(left<=0){btn.disabled=false;btn.textContent="📨 Reenviar código";clearInterval(resendTimer);return}btn.textContent="📨 Reenviar em "+left+"s";left--};tick();resendTimer=setInterval(tick,1000)}
 async function resend2FA(){if(!pendingEmail||!pendingPassword)return;const btn=$("resend2faBtn");setBusy(btn,true,"Enviando...");try{const r=await fetch(serverUrl()+"/api/2fa/resend",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:pendingEmail,password:pendingPassword})});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||"Não foi possível reenviar o código.");pendingChallenge=d.challenge;$("twoFactorCode").value="";setStatus(d.message||"Novo código enviado.","ok");$("twoFactorCode").focus();startResendCountdown(60)}catch(e){setStatus(e.message||"Não foi possível reenviar.");btn.disabled=false;btn.textContent="📨 Reenviar código"}}
 function back2FA(){pendingChallenge="";pendingEmail="";pendingPassword="";clearInterval(resendTimer);restoreAuth()}
 $("loginTab").onclick=()=>modeSet("login");$("registerTab").onclick=()=>modeSet("register");$("loginBtn").onclick=auth;
 $("verify2faBtn").onclick=verify2FA;$("resend2faBtn").onclick=resend2FA;$("back2faBtn").onclick=back2FA;
 ...
 $("twoFactorCode").addEventListener("input",()=>{const el=$("twoFactorCode");el.value=el.value.replace(/\D/g,"").slice(0,6);if(el.value.length===6)verify2FA()});
 $("twoFactorCode").addEventListener("paste",()=>setTimeout(()=>{const el=$("twoFactorCode");el.value=el.value.replace(/\D/g,"").slice(0,6);if(el.value.length===6)verify2FA()},0));
 [email,password,name,$("confirmPassword")].filter(Boolean).forEach(el=>el.addEventListener("keydown",e=>{if(e.key==="Enter"&&!$("twoFactorFields").classList.contains("hidden"))return;if(e.key==="Enter")auth()}));
 ...
 window.renderFriends=async()=>{let d;try{d=await api("/api/friends")}catch(e){d={friends:[],requests:[]}};[$("friendsList"),$("friendsAppList")].filter(Boolean).forEach(list=>{list.innerHTML="";(d.requests||[]).forEach(u=>{const x=document.createElement("div");x.className="friend-item";x.innerHTML='<span class="friend-dot"></span><div class="friend-info"><b></b><small></small></div><button>✓</button>';x.querySelector("b").textContent=u.name;x.querySelector("small").textContent=u.code+" • solicitação";x.querySelector("button").onclick=async()=>{await api("/api/friends/accept",{method:"POST",body:JSON.stringify({code:u.code})});renderFriends()};list.appendChild(x)});(d.friends||[]).forEach(u=>{const online=[...people.values()].some(p=>p.code===u.code);const x=document.createElement("div");x.className="friend-item";x.innerHTML='<span class="friend-dot"></span><div class="friend-info"><b></b><small></small></div><button>×</button>';x.querySelector(".friend-dot").classList.toggle("online",online);x.querySelector("b").textContent=u.name;x.querySelector("small").textContent=u.code+" • "+(online?"online":"offline");x.querySelector("button").onclick=async()=>{await api("/api/friends/remove",{method:"POST",body:JSON.stringify({code:u.code})});renderFriends()};list.appendChild(x)});if(!list.children.length)list.innerHTML='<span class="muted">Nenhum amigo ainda.</span>'})};
 async function addFriend(input,status){const code=input.value.trim().toUpperCase();if(!/^CL-[A-Z0-9]{6}$/.test(code)){status.textContent="Código inválido. Use CL-XXXXXX.";return}try{const d=await api("/api/friends/request",{method:"POST",body:JSON.stringify({code})});status.textContent=d.message||"Solicitação enviada!";input.value="";renderFriends()}catch(e){status.textContent=e.message}}
 $("addFriendBtn").onclick=()=>addFriend($("friendCodeInput"),$("friendStatus"));$("addFriendApp").onclick=()=>addFriend($("friendCodeApp"),$("friendAppStatus"));$("refreshFriends").onclick=window.renderFriends;$("friendsBtn").onclick=()=>{$("friendsPanel").classList.remove("hidden");renderFriends()};$("friendsClose").onclick=()=>$('friendsPanel').classList.add("hidden");$("copyUserCode").onclick=()=>navigator.clipboard?.writeText($("myCode").textContent);
 $("logoutBtn").onclick=()=>{localStorage.removeItem("conversaLiveToken");localStorage.removeItem("conversaLiveUser");location.href=location.pathname};
 $("createCallBtn").onclick=()=>{const c=Math.random().toString(36).slice(2,8).toUpperCase();$("roomCodeInput").value=c;openApp(c)};$("joinCallBtn").onclick=()=>{$("joinBox").classList.toggle("hidden");if(!$("joinBox").classList.contains("hidden"))$("roomCodeInput").focus()};$("confirmJoinBtn").onclick=()=>{const c=$("roomCodeInput").value.trim().toUpperCase();if(c)openApp(c)};
})();
const $=id=>document.getElementById(id);
const ICE={
  iceServers:[
...
function connect(){
  if(!window.io)loadSocketIO(); else startSocket();
}
function loadSocketIO(){
  const s=document.createElement("script");
  s.src=serverUrl()+"/socket.io/socket.io.js";
  s.onload=startSocket;
  s.onerror=()=>{$("status").textContent="Servidor não configurado";};
  document.head.appendChild(s);
}
function startSocket(){
  if(socket?.connected)return;
  socket=io(serverUrl(),{auth:{token:window.CONVERSA_TOKEN||localStorage.getItem("conversaLiveToken")},
  ...
  socket.on("connect_error",()=>{$("status").textContent="Servidor offline — tentando reconectar...";setConnectionLevel(0,"Offline")});
...
$("audioUnlock").onclick=unlockAllAudio;
