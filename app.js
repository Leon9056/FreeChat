...
window.renderFriends=async()=>{
 ...
 $("addFriendBtn").onclick=()=>addFriend($("friendCodeInput"),$("friendStatus"));$("addFriendApp").onclick=()=>addFriend($("friendCodeApp"),$("friendAppStatus"));$("refreshFriends").onclick=window.renderFriends;
 const closeFriendsPanel=()=>{const p=$("friendsPanel");if(!p)return;p.classList.add("hidden");p.setAttribute("aria-hidden","true");};
 const openFriendsPanel=()=>{const p=$("friendsPanel");if(!p)return;p.classList.remove("hidden");p.setAttribute("aria-hidden","false");renderFriends()};
 $("friendsBtn").onclick=()=>openFriendsPanel();
 $("friendsClose").onclick=(e)=>{e.preventDefault();e.stopPropagation();closeFriendsPanel();};
 $("friendsPanel")?.addEventListener("click",e=>{if(e.target===$("friendsPanel"))closeFriendsPanel();});
 document.addEventListener("keydown",e=>{if(e.key==="Escape")closeFriendsPanel();});
 $("copyUserCode").onclick=()=>navigator.clipboard?.writeText($("myCode").textContent);refreshUnreadCounts();
...
function startSocket(){
  ...
  socket.on("call-state",state=>{
    ...
    if(inCall && callReady && state.active){
      ...
      (Array.isArray(state.ready)?state.ready:[]).forEach(id=>{
        if(id!==socket.id){
          // A state arriving immediately after reconnect can contain a participant
          // who was already ready, before the live participant-ready event.
          setTimeout(()=>{
            if(typeof maybeConnectToPeer==="function") maybeConnectToPeer(id);
          },120);
        }
      });
    }
  ...
  function maybeConnectToPeer(id){
    if(!inCall||!callReady||!id||id===socket?.id||peers.has(id)||!socket?.connected)return;
    // Um único lado inicia cada par. Isso evita ofertas simultâneas, mas não
    // depende de quem é o criador da call. O socket com maior id sempre inicia.
    if(String(socket.id)>String(id)){
      createPeer(id,true).catch(err=>{
        console.warn("Falha ao criar peer",id,err);
        closePeer(id);
        // Uma tentativa curta de recuperação cobre o caso em que o outro
        // participante ficou pronto alguns ms depois da primeira sinalização.
        setTimeout(()=>maybeConnectToPeer(id),700);
      });
    }
  }
  ...
  socket.on("call-ready-users",ids=>{
    if(!inCall)return;
    (Array.isArray(ids)?ids:[]).forEach(maybeConnectToPeer);
  });

  socket.on("call-participant-ready",({id})=>{
    maybeConnectToPeer(id);
  });
...
$("markNotificationsRead")?.addEventListener("click",async()=>{try{await api("/api/notifications/read",{method:"POST"});loadNotifications()}catch(e){}});async function openCall(){
  ...
  joiningCall=true;
  // O painel global de amigos não pode ficar por cima da interface da chamada.
...
async function createPeer(id,initiator){
  ...
  // addTrack cria transceivers sendrecv. Reforçamos a direção para garantir
  // que o áudio remoto seja negociado mesmo quando a câmera está desligada.
  ...
  pc.ontrack=e=>{
    ...
    if(track.kind==="audio"){
      ...
      audio.onloadedmetadata=playRemote;
      playRemote();
