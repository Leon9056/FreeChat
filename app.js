/* Conversa Live v2.2.0 — sair do chat, parar de transmitir tela, tela cheia nos vídeos e mais melhorias */
...
$("loginTab").onclick=()=>modeSet("login");$("registerTab").onclick=()=>modeSet("register");$("loginBtn").onclick=auth;
 ...
 window.refreshUnreadCounts=refreshUnreadCounts; window.conversaApi=api; window.api=api;
...
async function applyMusicTrackToPeers(track,state){
 ...
 musicElement.addEventListener("error",()=>{setCallStatus("O áudio não pôde ser carregado. Tente outra música.","error")});
