/* Conversa Live v2.4.0 — áudio/música melhores, animações fluidas, otimizações e correções de bugs */
...
function showApp(d){
 ...
 window.CONVERSA_TOKEN=d.token;window.CONVERSA_USER=d.user;login.classList.add("hidden");menu.classList.remove("hidden");menu.classList.remove("menu-enter");void menu.offsetWidth;menu.classList.add("menu-enter");
...
$("loginTab").onclick=()=>modeSet("login");$("registerTab").onclick=()=>modeSet("register");$("loginBtn").onclick=auth;
 ...
 const t=localStorage.getItem("conversaLiveToken"),u=localStorage.getItem("conversaLiveUser");if(t&&u)try{window.CONVERSA_TOKEN=t;window.CONVERSA_USER=JSON.parse(u);login.classList.add("hidden");menu.classList.remove("hidden");menu.classList.remove("menu-enter");void menu.offsetWidth;menu.classList.add("menu-enter");$("welcomeName").textContent=window.CONVERSA_USER.name;$("myCode").textContent=window.CONVERSA_USER.code;$("sideCode").textContent=window.CONVERSA_USER.code;window.renderFriends?.();startFriendRequestPolling();connectLobby()}catch(e){localStorage.removeItem("conversaLiveToken");localStorage.removeItem("conversaLiveUser")}
 async function api(path,opts={}){const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),12000);try{const isForm=typeof FormData!=="undefined"&&opts.body instanceof FormData;const baseHeaders={Authorization:"Bearer "+(window.CONVERSA_TOKEN||localStorage.getItem("conversaLiveToken"))};if(!isForm)baseHeaders["Content-Type"]="application/json";const r=await fetch(serverUrl()+path,{...opts,signal:controller.signal,headers:{...baseHeaders,...(opts.headers||{})}});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||"Erro.");return d}catch(e){if(e?.name==="AbortError")throw Error("O servidor demorou demais para responder. Tente novamente.");throw e}finally{clearTimeout(timeout)}} 
...
window.renderFriends=async()=>{
 ...
 $("logoutBtn").onclick=()=>{clearInterval(friendPollTimer);try{socket?.disconnect?.();}catch(e){}localStorage.removeItem("conversaLiveToken");localStorage.removeItem("conversaLiveUser");location.href=location.pathname}; 
...
let musicAudioContext=null,musicElement=null,musicSource=null,musicDestination=null,musicMicSource=null,musicGainNode=null,musicTrack=null,musicHost=false,musicVolume=.7,musicState=null,musicMediaToken="";
...
/* FreeChat 2.4.0 — navegação por gesto no celular */
let mobileView=0;
...
/* FreeChat 2.4.0 — Social, notificações e PWA */
let socialLoaded=false;
function openSocial(tab="feed"){const panel=$("socialPanel");if(!panel)return;panel.classList.remove("hidden");switchSocialTab(tab);if(tab==="feed")loadFeed();if(tab==="friends")window.renderFriends?.();if(tab==="notifications")loadNotifications();if(tab==="profile")loadProfile();}
...
async function loadFeed(){try{const d=await api("/api/feed");const list=$("feedList");if(!list)return;if(!d.posts?.length){list.innerHTML='<div class="social-empty">📰<b>Seu feed está vazio</b><span>Publique algo ou adicione amigos para começar.</span></div>';return}list.innerHTML=d.posts.map(p=>{const safe=messageEscape(p.body);const initial=messageEscape((p.name||"?").trim().charAt(0).toUpperCase());const date=new Date(p.created_at).toLocaleString([],{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"});return `<article class="post-card"><div class="post-head"><div class="post-avatar">${initial}</div><div><b>${messageEscape(p.name)}</b><small>${messageEscape(p.code)} · ${date}</small></div></div><div class="post-body">${safe.replace(/\n/g,"<br>")}</div><button class="post-like ${p.liked?"liked":""}" data-like="${p.id}">❤️ <span>${Number(p.likes||0)}</span></button></article>`}).join("");list.querySelectorAll("[data-like]").forEach(b=>b.onclick=async()=>{try{const d=await api("/api/feed/"+b.dataset.like+"/like",{method:"POST"});b.classList.toggle("liked",d.liked);b.querySelector("span").textContent=d.likes}catch(e){appToast(e.message,"error")}})}catch(e){appToast(e.message,"error")}}
...
$("socialBtn")?.addEventListener("click",()=>openSocial("feed"));$("socialClose")?.addEventListener("click",()=>$("socialPanel")?.classList.add("hidden"));$("saveProfileBtn")?.addEventListener("click",saveProfile);$("socialRefreshFriends")?.addEventListener("click",()=>renderFriends());$("socialFriendsSearch")?.addEventListener("input",e=>{window.friendSearchTerm=e.target.value;renderFriends()});$("postBody")?.addEventListener("input",e=>$("postCount").textContent=e.target.value.length+"/1000");document.querySelectorAll(".social-tab").forEach(b=>b.addEventListener("click",()=>{switchSocialTab(b.dataset.socialTab);const t=b.dataset.socialTab;if(t==="feed")loadFeed();if(t==="friends")renderFriends();if(t==="notifications")loadNotifications();if(t==="profile")loadProfile()}));$("postForm")?.addEventListener("submit",async e=>{e.preventDefault();const body=$("postBody").value.trim();if(!body)return;try{await api("/api/feed",{method:"POST",body:JSON.stringify({body})});$("postBody").value="";$("postCount").textContent="0/1000";loadFeed()}catch(err){appToast(err.message,"error")}});$("markNotificationsRead")?.addEventListener("click",async()=>{try{await api("/api/notifications/read",{method:"POST"});loadNotifications()}catch(e){}});
...
async function createPeer(id,initiator){
  ...
  // Tune the audio sender for stable conversational voice (or higher quality while music plays).
  try{
    const sender=pc.getSenders().find(s=>s.track?.kind==="audio");
    if(sender){
      const p=sender.getParameters();
      p.encodings=p.encodings?.length?p.encodings:[{}];
      p.encodings[0].maxBitrate=musicHost?160000:64000;
      p.encodings[0].networkPriority="high";
...
function enterOrExitTileFocus(tile){
  ...
  if(tile.webkitRequestFullscreen){tile.webkitRequestFullscreen();return;}
  const video=tile.querySelector("video");
  if(video?.webkitEnterFullscreen){video.webkitEnterFullscreen();return;}
  fallbackTileFocus(tile);
}
...
/* FreeChat 2.4.0 — chat privado com fotos, vídeos e notificações */
...
$("messageFile")?.addEventListener("change",e=>{const f=e.target.files?.[0];if(!f)return;const ok=/^(image\/|video\/)/i.test(f.type);if(!ok){appToast("Escolha uma foto ou vídeo válido.","error");e.target.value="";return}if(f.size>20*1024*1024){appToast("O arquivo precisa ter no máximo 20 MB.","error");e.target.value="";return}setSelectedMessageFile(f)});
